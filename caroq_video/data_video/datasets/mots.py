...
__all__ = ["load_mots_json", "register_mots_instances"]

MOTS_CATEGORIES= [
    {'id': 0, 'name': 'car', 'supercategory': 'car', 'isthing': 1, 'color': [128, 64, 128]},
    {'id': 1, 'name': 'pedestrian', 'supercategory': 'pedestrian', 'isthing': 1, 'color': [244, 35, 232]},
    {'id': 2, 'name': 'ambiguous', 'supercategory': 'ambiguous', 'isthing': 1, 'color': [70, 70, 70]}]


def _get_mots_instances_meta():

    thing_ids = [k["id"] for k in MOTS_CATEGORIES]
    thing_colors = [k["color"] for k in MOTS_CATEGORIES]
    thing_classes = [k["name"] for k in MOTS_CATEGORIES]


    assert len(thing_ids) == 3
...
def register_mots_instances(name, metadata, json_file, image_root):
    ...
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="mots", **metadata
    )
