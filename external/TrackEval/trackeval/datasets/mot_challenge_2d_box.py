...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, '../../../data/MOTChallenge/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, "../../../output/MOT17/train/tracking/FRCNN_withoptflow/losses_tracklambda/test/"),  # Trackers location
            #'TRACKERS_FOLDER': os.path.join(code_path, "../../../output/MOT20/other_trackers/"),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['pedestrian'],  # Valid: ['pedestrian']
            'BENCHMARK': 'MOT17',  # Valid: 'MOT17', 'MOT16', 'MOT20', '2DMOT2015'
            'SPLIT_TO_EVAL': 'train',  # Valid: 'train', 'test', 'all'
            'INPUT_AS_ZIP': False,  # Whether tracker input files are zipped
            'PRINT_CONFIG': True,  # Whether to print current config
            'DO_PREPROC': True,  # Whether to perform preprocessing (never done for 2DMOT2015)
            'TRACKER_SUB_FOLDER': 'Instances_txt',  # Tracker files are in TRACKER_FOLDER/tracker_name/TRACKER_SUB_FOLDER
            'OUTPUT_SUB_FOLDER': '',  # Output files are saved in OUTPUT_FOLDER/tracker_name/OUTPUT_SUB_FOLDER
            'TRACKER_DISPLAY_NAMES': None,  # Names of trackers to display, if None: TRACKERS_TO_EVAL
            'SEQMAP_FOLDER': "../MOTChallengeEvalKit/seqmaps/",  # Where seqmaps are found (if None, GT_FOLDER/seqmaps)
            'SEQMAP_FILE': "../MOTChallengeEvalKit/seqmaps/MOT17-train_FRCNN1.txt",  # Directly specify seqmap file (if none use seqmap_folder/benchmark-split_to_eval)
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.do_preproc and self.benchmark != '2DMOT2015' and gt_ids.shape[0] > 0 and tracker_ids.shape[0] > 0:
            ...
            # class (not applicable for 2DMOT2015)
            if self.benchmark != '2DMOT2015':
                gt_to_keep_mask = (np.not_equal(gt_zero_marked, 0)) & \
                                  (np.equal(gt_classes, cls_id))
            else:
                # There are no classes for 2DMOT2015
                gt_to_keep_mask = np.not_equal(gt_zero_marked, 0)
