...
class KittiMOTS(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, '../../data/KITTI_MOTS/val'),  # Location of GT data
            #'TRACKERS_FOLDER': os.path.join(code_path, "Results/KITTI_MOTS_val/"),  # Trackers location
            'TRACKERS_FOLDER': os.path.join(code_path, "../../output/KITTI_MOTS/val/tracking/pointtrack_online_phi3/losses_tracklambda/test/"),  # Trackers location
            ...
            #'TRACKER_SUB_FOLDER':"tracks_car_pointtrack_val",  # Tracker files are in TRACKER_FOLDER/tracker_name/TRACKER_SUB_FOLDER
            'OUTPUT_SUB_FOLDER': '',  # Output files are saved in OUTPUT_FOLDER/tracker_name/OUTPUT_SUB_FOLDER
            'TRACKER_DISPLAY_NAMES': None,  # Names of trackers to display, if None: TRACKERS_TO_EVAL
            'SEQMAP_FOLDER': "../mots_tools/mots_eval",  # Where seqmaps are found (if None, GT_FOLDER)
            'SEQMAP_FILE': "../mots_tools/mots_eval/val_small2.seqmap",    # Directly specify seqmap file (if none use seqmap_folder/split_to_eval.seqmap)
