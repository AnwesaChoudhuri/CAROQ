...
class MOTSChallenge(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, '../../../data/MOTS_Challenge/val'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path,  "../../../output/MOTS_Challenge/val/tracking/"),  # Trackers location
            ...
            'SEQMAP_FOLDER': "../mots_tools/mots_eval",  # Where seqmaps are found (if None, GT_FOLDER/seqmaps)
            'SEQMAP_FILE':"../mots_tools/mots_eval/val_MOTSchallenge.seqmap",  # Directly specify seqmap file (if none use seqmap_folder/MOTS-split_to_eval)
            'SEQ_INFO': None,  # If not None, directly specify sequences to eval and their number of timesteps
            'GT_LOC_FORMAT': '{gt_folder}/instances_txt/{seq}/gt/gt.txt',  # '{gt_folder}/{seq}/gt/gt.txt'
    ...
    def __init__(self, config=None):
        ...
        self.tracker_fol = os.path.join(self.config['TRACKERS_FOLDER'], self.config['TRACKERS_TO_EVAL'][0])
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    seq = row[0]
                    seq_list.append(seq)
                    ini_file = os.path.join(self.gt_fol, "seqinfo", seq, 'seqinfo.ini')
