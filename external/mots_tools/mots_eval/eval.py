...
sys.path.append('../')
import csv
#from io import load_seqmap, load_sequences
...
def compute_MOTS_metrics_per_sequence(seq_name, gt_seq, results_seq, max_frames, class_id,
  ...
  for f in range(max_frames + 1):
    ...
    for gg in g:
      ...
      seq_tr_id[gg.track_id].append((f))
    num_associations = 0
    ...
    for row, gg in enumerate(g):
      ...
      for col, tt in enumerate(t):
        ...
        #print(gg.track_id,tt.track_id,c,rletools.area(tt.mask))
        clist.append(c)
        #print(clist)
        ...
        if c > 0.5:
          ...
          seq_tr_id[g[row].track_id][-1]=f
          #print (col)
          abcd=10
          #print(rletools.area(tt.mask),seq_trajectories)
  ...
  if len(seq_trajectories) != 0:
    for i, (g,frame_no) in enumerate(zip(seq_trajectories.values(),seq_tr_id.values())):
      ...
      for f in range(1, len(g)):
        if last_id != g[f] and last_id != -1 and g[f] != -1:
          ...
          ids_frames.append(["Frames: ",frame_no[np.where(np.array(g)==last_id)[0].max()],frame_no[f],"  id switch: ",last_id, g[f]])
  ...
  return results_obj, ids_frames


def compute_MOTS_metrics(gt, results, max_frames, class_id, ignore_class, overlap_function, list_all):
  """
  ...
  for seq in gt.keys():
    ...
    results_per_seq[seq], ids_frames_all[seq] = compute_MOTS_metrics_per_sequence(seq, gt[seq], results_seq, max_frames[seq], class_id,
                                                             ignore_class, overlap_function)
  ...
  lim = results_folder.find("Instances")
  list_all=print_summary(list(gt.keys()), results_per_seq, results_for_all_seqs,list_all)

  return results_per_seq, results_for_all_seqs,list_all, ids_frames_all
...
def print_summary(seq_names, results_per_seq, results_for_all_seqs, column_width=14,list_all=[]):
...
def evaluate_class(gt, results, max_frames, class_id,list_all):
  results_seq, results_obj,list_all, ids_all = compute_MOTS_metrics(gt, results, max_frames, class_id, IGNORE_CLASS, mask_iou,list_all)
  return results_seq, results_obj,list_all, ids_all
...
def run_eval(results_folder, gt_folder, seqmap_filename):
  ...
  list_all=[]
  results_seq_cars, results_cars,list_all, ids_all_car = evaluate_class(gt, results, max_frames, 1,list_all)
  print("Evaluate class: Pedestrians")
  list_all.append(["Pedestrians"])
  results_seq_ped, results_ped , list_all, ids_all_ped= evaluate_class(gt, results, max_frames, 2,list_all)
  lim=results_folder.find("Instances")


  with open(results_folder[:lim]+'results.csv',"w") as f:
    writer = csv.writer(f)
    writer.writerow(["Cars"])
    for i in list_all:
      writer.writerow(i)

    with open(results_folder[:lim] + 'ID_switches.txt', "w") as f:
      writer = csv.writer(f)
      writer.writerow(["Cars"])
      for i in list(ids_all_car.keys()):
        writer.writerow(i)
        for line in ids_all_car[i]:
          writer.writerow(line)

      writer.writerow(["Pedestrians"])
      for i in list(ids_all_ped.keys()):
        writer.writerow(i)
        for line in ids_all_ped[i]:
          writer.writerow(line)
    # print("CAR",file=f)
    # print(" ", "sMOTSA","MOTSA", "MOTSP",file=f)
    # print(" ", results_cars.sMOTSA*100, results_cars.MOTSA*100, results_cars.MOTSP*100, file=f)
    # print("PEDESTRIAN", file=f)
    # print(" ", results_ped.sMOTSA*100, results_ped.MOTSA*100, results_ped.MOTSP*100, file=f)
