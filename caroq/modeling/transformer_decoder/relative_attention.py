...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, heads: int, d_model: int, dropout: float = 0.0, bias: bool = True):
        """
        ...
        * `d_model` is the number of features in the `query`, `key` and `value` vectors.
        ...
        self.in_proj_weight=nn.Parameter(torch.empty(self.d_model*3, self.d_model), requires_grad=True)
        self.in_proj_bias=nn.Parameter(torch.empty(self.d_model*3), requires_grad=True)
    ...
    def get_scores(self, query: torch.Tensor, key: torch.Tensor, key_pos=None, query_pos=None,pos_embeddings=None, rel_scale=0):
        """
    ...
    def forward(self, *,
                ...
                key: torch.Tensor,
                value: torch.Tensor,
                mask: Optional[torch.Tensor] = None, key_pos=None, query_pos=None,pos_embeddings=None, rel_scale=0):
        """
        ...
        query = self.query(query, weight=in_proj_weight[self.modes["query"]*self.d_model:(self.modes["query"]+1)*self.d_model],
                                    bias=in_proj_bias[self.modes["query"]*self.d_model:(self.modes["query"]+1)*self.d_model])


        key = self.key(key, weight=in_proj_weight[self.modes["key"]*self.d_model:(self.modes["key"]+1)*self.d_model],
                                    bias=in_proj_bias[self.modes["key"]*self.d_model:(self.modes["key"]+1)*self.d_model])

        value = self.value(value, weight=in_proj_weight[self.modes["value"]*self.d_model:(self.modes["value"]+1)*self.d_model],
                                    bias=in_proj_bias[self.modes["value"]*self.d_model:(self.modes["value"]+1)*self.d_model])

        del in_proj_bias, in_proj_weight

        scores = self.get_scores(query, key, key_pos=key_pos, query_pos=query_pos, pos_embeddings=pos_embeddings, rel_scale=rel_scale)
...
class RelativeMultiHeadAttention(MultiHeadAttention):
    ...
    def __init__(self, heads: int, d_model: int, dropout: float = 0.1, P1=100, P2=100):
        ...
        self.key_pos_embeddings = nn.Parameter(torch.empty(self.P1 + self.P2, heads, self.d_k), requires_grad=True)
    ...
    def get_scores(self, query: torch.Tensor, key: torch.Tensor, key_pos=None, query_pos=None, pos_embeddings=None, rel_scale=0):
        ...
        if pos_embeddings==None:
            key_pos_emb =self.key_pos_embeddings[self.P1 - key.shape[0]:self.P1 + query.shape[0]]
        else:
            key_pos_emb =pos_embeddings.weight.view(pos_embeddings.weight.shape[0], self.heads, self.d_k)[self.P - key.shape[0]:self.P + query.shape[0]]
