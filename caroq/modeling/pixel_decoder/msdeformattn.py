...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.transformer_in_features = [
            k for k, v in transformer_input_shape
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ] = 1024  # use 1024 for deformable transformer encoder
        ret[
        ...
        if ret["time_gap"] == 1:
            ret["pe_layer"] = "2D"  # cfg.MODEL.MASK_FORMER.POSITIONAL_ENCODING
        else:
            ret["pe_layer"] = "2D"  # cfg.MODEL.MASK_FORMER.POSITIONAL_ENCODING
        return ret
    ...
    def forward_features(self, features):
        ...
        for idx, f in enumerate(self.transformer_in_features[::-1]):
            ...
            if self.pe_layer_from_config == "3D":
                bs = int(x.shape[0] / self.time_gap)
                x_temp = self.input_proj[idx](x)
                x_temp = x_temp.view(
                    bs, self.time_gap, x_temp.shape[1], x_temp.shape[2], x_temp.shape[3]
                )

                # bs,t,c,h,w => bs,c,t,h,w => bs,c,t*h,w
                srcs.append(x_temp.permute(0, 2, 1, 3, 4).flatten(2, 3))
                pos.append(self.pe_layer(x_temp).permute(0, 2, 1, 3, 4).flatten(2, 3))
            else:
                srcs.append(self.input_proj[idx](x))
                pos.append(self.pe_layer(x))

        y, spatial_shapes, level_start_index = self.transformer(srcs, pos)
        if self.pe_layer_from_config == "2D":
            bs = y.shape[0]
        ...
        for i, z in enumerate(y):
            if self.pe_layer_from_config == "3D":
                # bs,t*h*w,c => bs,c,t*h*w => bs,c,t,h,w
                z_temp = z.transpose(1, 2).view(
                    bs,
                    -1,
                    self.time_gap,
                    int(spatial_shapes[i][0] / self.time_gap),
                    spatial_shapes[i][1],
                )
                # bs,c,t,h,w => bs,t,c,h,w, bs*t,c,h,w
                out.append(z_temp.permute(0, 2, 1, 3, 4).flatten(0, 1))
            else:
                out.append(
