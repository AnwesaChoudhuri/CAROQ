...
def get_mots_dict(data_dir, subset=False):
    ...
    idx=0
    record={}
    record["categories"]=[{"id": 0, "name": "car"},
    ...
    for dir1 in all_dirs:
        print(dir1)
        if subset==True:
            image_names = sorted(os.listdir(os.path.join(data_dir, "images", dir1)))[:20]
        else:
            image_names = sorted(os.listdir(os.path.join(data_dir, "images", dir1)))
        if os.path.exists(os.path.join(data_dir, "instances_txt")):
            objects_per_frame = io1.load_txt(os.path.join(data_dir, "instances_txt", dir1, "gt", "gt.txt"))
        else:
            objects_per_frame=[]
        ...
        file_names=[]
        image_ids=[]
        unique_track_ids=list(set([ob.track_id for k in objects_per_frame.keys() for ob in objects_per_frame[k] if k in [int(img[:-4]) for img in image_names]]))

        labels={k: {"video_id": idx,
                    "vid": dir1,
        ...
        for i in range(0,len(image_names)):
            ...
            for obj_id in unique_track_ids:
                ...
                if objects_per_frame!=[] and int(image_name[:-4]) in list(objects_per_frame.keys()) and obj_id in [
                                                        k.track_id for k in objects_per_frame[int(image_name[:-4])]]:
                    objects=[k for k in objects_per_frame[int(image_name[:-4])] if k.track_id==obj_id]
                    if len(objects)==1:
                        obj=objects[0]
                        ...
                        labels[obj_id]["bboxes"].append([cocomask.toBbox(obj.mask)[0],
                        ...
                        labels[obj_id]["category_id"]=obj.class_id-1 if obj.class_id in [1,2] else 2 #car, ped, ignore
                        ...
                        mask=np.zeros((labels[obj_id]["height"],labels[obj_id]["width"]), dtype=bool)
                        for obj in objects:
...
get_all_mots_dict2(data_dir="data/MOTS/train", save_file="data/MOTS/train_full.json", subset=False)
