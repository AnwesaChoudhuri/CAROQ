...
videos = np.unique(
    [a["file_name"][: a["file_name"].find("_")] for a in json_data["images"]]
)
...
for idx, v in enumerate(videos):
    ...
    for annos in anno_files:
        ...
        for u_idx, u in enumerate(unique_ids):
            ...
            unique_objects[u_idx]["segmentations"].append(
                {"size": seg["size"], "counts": seg["counts"].decode(encoding="UTF-8")}
            )
